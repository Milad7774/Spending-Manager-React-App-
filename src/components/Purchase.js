    ...
    return(
        <div>
            <div className="div-table">
                ...
                {sortedList.map((item, index) =>(<tr key={index}><td>{item.bought}</td><td>{formatPrice(item.money)} SYP</td><td>{item.date}</td><td><span onClick={() => onDelete(index)}>&#x274C;</span></td></tr>))}
                </tbody>
                </table>
            </div>
            ...
            <div className="total">
                ...
                <div className="total-number" >{ formatPrice(total.toString()) } SYP</div>
                </div>
